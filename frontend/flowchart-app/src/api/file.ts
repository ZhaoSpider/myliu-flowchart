...
export const fileApi = {
  ...
  getContent: (id: number) => 
    request.get<Result<string>>(`/files/${id}/content`),
   ...
   * 导出文件
   */
  export: (id: number, format: 'png' | 'svg' | 'json'): Promise<AxiosResponse<Blob>> => 
    request.get<AxiosResponse<Blob>>(`/files/${id}/export`, { 
      params: { format },
      responseType: 'blob'
    } as any)
}
